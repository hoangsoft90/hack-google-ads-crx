...
if(isset($_POST['login'])){
    ...
    $demo=login_demo1($_POST['email'],$_POST['pass']);
    if($demo){
        if(!$demo['verify']){
            ...
            if(0 && send_mail($_POST['email'],$subject,$body)){
                $logs[]='Vui lòng kiểm tra email để kích hoạt tài khoản.';
            }
            $logs[]='Mật khẩu của bạn là: '.$code;
        }
...
<form method="post" action="demo0.php">
    <label for="email">Email</label><input type="text" name="email" value=""/>
    <label for="pass">Pass</label><input type="text" name="pass" value=""/>
...
?>
<!-- form -->
<form method="post" name="frm_keywords" action="demo0.php">
<table>
<?php
